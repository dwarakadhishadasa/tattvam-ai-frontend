...
- [ ] Upgrade the notebook entry data model and derived helpers (AC: 1, 2, 6, 7, 8)
  - [ ] Replace the string-only `SavedSnippet` shape in `components/pipeline/types.ts` with the approved notebook-entry fields: `id`, `sourceMessageId`, `sourceType`, `sourceContent`, `content`, `isEdited`, `updatedAt`.
  - [ ] Add `activeNotebookEntryId` to `SessionState` and `createEmptySessionState()`.
  - [ ] Update `components/pipeline/utils.ts` so word count and compile source use edited entry `content`, not immutable `sourceContent`.
  - [ ] Keep `canCompile` and `notebookReadiness` derived from current notebook entries instead of creating new persisted booleans.

- [ ] Replace string-only save APIs with provenance-aware notebook actions (AC: 2, 3, 6, 8)
  - [ ] Change save handlers in `components/pipeline/PipelinePageClient.tsx` and modal/step props from `onSaveSnippet(content)` to richer save actions that preserve source metadata.
  - [ ] Save assistant responses with `sourceType: "response"` and `sourceMessageId` set to the assistant message id.
  - [ ] Save citation excerpts with `sourceType: "citation"` and preserve provenance as far as the current modal contract allows.
  - [ ] Save context blocks with `sourceType: "context"`.
  - [ ] Deduplicate initial saves by exact `sourceContent`, not by mutable edited `content`.

- [ ] Fold notebook review, editing, readiness, naming, and compile into extraction UI (AC: 1, 2, 3, 4, 5, 6, 7)
  - [ ] Refactor `components/pipeline/PipelineSteps.tsx` so `ExtractionStep` owns the notebook list, editor/detail panel, readiness indicator, workspace name input, and compile controls.
  - [ ] Remove the separate synthesis-stage dependency from the user flow.
  - [ ] Implementation decision: use a dedicated editor/detail panel rather than inline card editing so chat scroll and notebook list stability are preserved.
  - [ ] Implementation decision: place workspace naming and compile controls together in the notebook footer area.
  - [ ] Implementation decision: closing the editor clears `activeNotebookEntryId`.

- [ ] Update the step model, navigation, and compile handoff (AC: 4, 5, 6)
  - [ ] Change `PipelineStep` from `0 | 1 | 2 | 3` to `0 | 1 | 2`.
  - [ ] Update `components/pipeline/constants.ts` and `components/pipeline/PipelineSidebar.tsx` to show `Context`, `Extraction`, and `Presentation`.
  - [ ] Stop rendering `SynthesisStep` from `components/pipeline/PipelinePageClient.tsx`; remove dead code if no longer needed.
  - [ ] Keep compile behavior in `handleGenerateNotebook()` but advance directly from extraction to presentation.

- [ ] Align persistence and restore behavior with notebook edits without reintroducing raw storage calls (AC: 2, 8)
  - [ ] Extend `hooks/useSessionPersistence.ts`, `lib/persistence/schema.ts`, and any related validators to accept the new notebook-entry structure and reduced step count.
  - [ ] Normalize legacy saved-snippet records into notebook entries during restore if encountered, but do not add migration-specific UI or separate migration flows.
  - [ ] Keep persistence ownership inside the existing hook and `lib/persistence/*`; do not move storage logic back into `PipelinePageClient.tsx`.

- [ ] Verify compile and presentation still consume edited notebook content (AC: 4, 5, 7, 8)
  - [ ] Ensure notebook compile availability is `savedSnippets.length > 0`.
  - [ ] Ensure slide generation continues to use edited notebook `content`.
  - [ ] Extend existing persistence tests and add any small pure-helper tests needed for notebook-entry restore/validation.
